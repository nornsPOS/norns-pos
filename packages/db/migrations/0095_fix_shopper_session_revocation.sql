-- ═════════════════════════════════════════════════════════════════════════
-- 0095 — repair 0094: shopper_sessions has no revoked_at.
-- ═════════════════════════════════════════════════════════════════════════
--
-- 0094 added the shopper scrub to erase_customer() and ended it by revoking
-- open sessions with `UPDATE shopper_sessions SET revoked_at = now()`. That
-- column does not exist on this table. Session revocation (0089) added
-- revoked_at to the STAFF sessions table; the shopper table was never given
-- one, because a shopper session is a bearer token with an expiry and nothing
-- more.
--
-- The consequence was immediate and total: erase_customer() threw for EVERY
-- caller, so between 0094 and this migration the staff Art. 17 button was
-- broken. Deleting the session rows outright is both correct and stronger
-- than marking them revoked.
--
-- Caught by calling the customer's own erasure endpoint against production
-- with a real account holding a real reservation. A migration that applies
-- cleanly is not a migration that works: CREATE OR REPLACE FUNCTION only
-- parses the body, it never checks that the columns exist.
-- ═════════════════════════════════════════════════════════════════════════
